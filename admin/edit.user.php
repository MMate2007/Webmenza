...
if (isset($_POST["name"])) {
    $stmt = $mysql->prepare("UPDATE `users` SET `name`=?,`groupId`=?,`registered`=?,`dietId`=? WHERE `id` = ?");
    $group = ($_POST["group"] == "") ? null : (int)$_POST["group"];
    $registered = $_POST["registered"] ?? 0;
    $stmt->bind_param("siiii", $_POST["name"], $group, $registered, $_POST["diet"], $_GET["id"]);
    $stmt->execute();
    Message::addMessage("Sikeres mentés!", MessageType::success);
}
$stmt = $mysql->prepare("SELECT `users`.`name`, `groupId`, `registered`, `dietId` FROM `users` WHERE `users`.`id` = ?");
...
echo $twig->render("edit.user.html.twig", ["user" => $user, "groups" => $groups, "diets" => $diets]);
?>