...
$stmt->execute();
$stmt = $mysql->prepare("DELETE `deadlines`, `modifications` FROM `deadlines` INNER JOIN `modifications` ON `modifications`.`date` BETWEEN `deadlines`.`from` AND `deadlines`.`to` WHERE `deadlines`.`id` = ?");
